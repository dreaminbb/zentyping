<template>
  <div id="app">
    <div id="buttons" class="buttons">
      <button id="short" class="short">short</button>
      <button id="normal" class="normal">normal</button>
      <button id="long" class="long">long</button>
    </div>
    ...
    <div id="container" class="container">
      ...
      <div id="charDisplay" class="charDisplay" contenteditable="true"></div>
    </div>
    <div id="others" class="others">
      ...
      <button id="color" class="color"></button>
      <button id="setting" class="setting"></button>
    </div>
...
function baseofgame() {
  ...
  typeInput.addEventListener('input', () => {
    ...
    function end() {
      ...
      accvalue = Math.round(((correct - trueincorrect) / charDisplay.innerText.length) * 100)
      setTimeout(function () {
        wpm.innerText = wpmvalue
        acc.innerText = accvalue + '%'
        timeDisplay.innerText = seconds + 1 + 's'
        allinputDisplay.innerText =
          typeInput.value.length + '/' + correct + '/' + incorrect + '/' + trueincorrect
...
.active {
  ...
  box-shadow: 0 5px 20px rgba(215, 255, 250, 0.7);
  border-color: transparent;
}
...
.others .about {
  /* 既存のスタイル */
  margin: 0 60px; /* 上下に0px、左右に10pxのマージンを設定 */
  border-radius: 20px;
  border: none;
  color: #d4d4d4;
}

.others .restart:hover,
.others .write:hover,
.others .about:hover,
.others .restart:focus,
.others .write:focus,
.others .about:focus {
  cursor: pointer;
...
.others .restart:hover::before,
.others .restart:focus::before {
  content: 'Tab or click で再スタート';
  white-space: pre;
  position: absolute;
  ...
  border-radius: 5px;
  top: -70px; /* 吹き出しの位置を調整 */
  left: 230px; /* 吹き出しの位置を調整 */
  font-size: 1.5rem;
  overflow-wrap: break-word;
  border-radius: 9px;
}

.others .restart:hover::after,
.others .restart:focus::after {
  content: '';
  position: absolute;
  top: -26px; /* 矢印の位置を調整 */
  left: 30%; /* 矢印の位置を調整 */
...
.resultother .allinput,
.resultother .time,
.resultother .allinputDisplay,
.resultother .timeDisplay {
  color: #949494;
  justify-content: relative;
  position: absolute;
  display: flex;
