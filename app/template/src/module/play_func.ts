...
export class play_func {
    ...
    private type_counter: number;
    private time_value: number;
    private char_all_spans_as_array_elm: Array<HTMLElement>;
    private essenced_spans_for_comparison: Array<HTMLSpanElement>;
    public result_data: result_data_itf;
    private char_index: number;
    private timer_score_watcher_func: number;
    public line_index: number;
    public timer_func: number;
    public char_spans: Array<HTMLElement>;
    public splited_code_arr: Array<string>;
    private time_display_display: HTMLElement | null;
    public wpm_every_second_arr: Array<number>;
    public acc_every_secend_arr: Array<number>;
...
<<<<<<< HEAD
  ...
  private format_to_ignore_space_after_line_break(
    char_span_arr: NodeListOf<HTMLSpanElement>,
  ): any {
...
=======
    public delete(): void {
        ...
        window.removeEventListener(
            "keydown",
            this.handle_keydown_for_play.bind(this),
        );
        this.time_display_display = document.getElementById(
            "time_display",
        ) as HTMLElement;
...
<<<<<<< HEAD
  ...
  public fetch_char_spans_ignore_space_after_line_break_as_elm(): void {
    ...
    const char_spans_value = (this.play_code_display_container as HTMLElement).querySelectorAll(".each_char_elm");
  ...
  private comparison_input_and_add_class_to_span(input_char: string): void {
    ...
    const is_target_elm_text_space: boolean =
      target_span_elm.textContent === "\u00A0";
...
<<<<<<< HEAD
const play_func_inc = new play_func();
export default play_func_inc;
=======
    private is_include_incorrect_class_in_ess_spans(): boolean {
        try {
            for (const value of this.essenced_spans_for_comparison) {
        ...
        try {
            for (const value of this.essenced_spans_for_comparison) {
                ...
                const array_from_values_class_name: Array<string> =
                    value.className.split(" ");
                if (array_from_values_class_name.includes("untyped")) {
                    // console.log(array_from_values_class_name, value.textContent, ' this is the untyped')
                    return false;
                }
            }
            return true;
    ...
    private format_to_ignore_space_after_line_break(
        ...
        try {
            ...
            if (!char_span_arr_decoy) throw new Error('char_span_arr_decoy is undifid')

            let space_index: number = 0;
            let space_start_point: number = 0;
            let space_end_point: number = 0;
            for (let i = 0; i < char_span_arr.length; i++) {
                if (char_span_arr[i]?.textContent === undefined) return
                if (
                    char_span_arr[i]?.textContent === "\u00A0" &&
                    char_span_arr[i - 1]?.textContent === "\n"
                ) {
                    ...
                    //* store spans include \u00A0.
                    for (let j = space_start_point; j < space_end_point; j++) {
                        removed_spans.push(char_span_arr_decoy[j] as HTMLSpanElement);
                    }
                }
            }
    ...
    public fetch_char_spans_ignore_space_after_line_break_as_elm(): void {
        ...
        const char_spans_value = (
            this.play_code_display_container as HTMLElement
        ).querySelectorAll(".each_char_elm");
    ...
    private comparison_input_and_add_class_to_span(input_char: string): void {
        const target_span_elm = this.essenced_spans_for_comparison[this.char_index];
        const before_target_span_elm =
            this.essenced_spans_for_comparison[this.char_index - 1];
